...
1. Build the source firmware (current version) and flash it to your device::
      ...
      west build -b nrf7002dk/nrf5340/cpuapp samples/subsys/mgmt/delta/ --sysbuild -p
...
3. Apply the modification you want to add to your current firmware::

      git apply samples/subsys/mgmt/delta/patches/create_target_firmware.patch

4. Build the target firmware::

      west build -b nrf7002dk/nrf5340/cpuapp samples/subsys/mgmt/delta/ --sysbuild -p

5. Store your target binary::

      cp build/delta/zephyr/zephyr.signed.bin samples/subsys/mgmt/delta/firmwares/target.bin

6. Use the provided Python scripts to generate the patch::
...
- :kconfig:option:`CONFIG_BSDIFF_HS_ALGO`: Use bsdiff algorithm and heatshrink compression for patching
