...
struct bspatch_stream
{
   ...
   size_t              offset;
   int (*decompress)(struct bspatch_stream *stream,
                     struct delta_api_t    *self_p,
 ...
 */
static int decompress_patch(struct bspatch_stream *stream,
                            struct delta_api_t    *self_p,
                            uint8_t               *output_buffer,
                            size_t                 output_buffer_size,
                            heatshrink_decoder    *hsd,
   ...
   while(true)
   {
      switch(*state)
      {
         case STATE_SINK:
            ...
            if(ret != 0)
            {
               LOG_ERR("Can not read %d bytes in slot1 partition at offset : %lld, ret = %d\n",
                       INPUT_READ_SIZE,
                       self_p->memory.offset.patch,
                       ret);
               return ret;
            }
 
            /* update the offset in the patch */
            ret = self_p->seek(&self_p->memory,
                               self_p->memory.offset.source,
                               self_p->memory.offset.patch + INPUT_READ_SIZE,
                               self_p->memory.offset.target);
            if(ret != 0)
            {
               LOG_ERR("Can not seek at offset for source : %d and for patch : %lld, ret = %d\n",
                       0,
                       self_p->memory.offset.patch + INPUT_READ_SIZE,
                       ret);
               return ret;
            ...
            HSD_sink_res sink_res = heatshrink_decoder_sink(hsd, input_buffer, INPUT_READ_SIZE, &read_sz);
            if(sink_res != 0)
            {
               if(sink_res == 1)
               {
         ...
         case STATE_POLL:
            ...
            {
               size_t to_decode = output_buffer_size - offset_buffer < OUTPUT_READ_SIZE
...
static int64_t offtin(uint8_t *buf)
{
   ...
   /* Start with the last byte (buf[7]) and use a mask (0x7F) to keep only the lower 7 bits (ignoring the sign
    * bit)*/
   y  = buf[7] & 0x7F;
   /* Accumulate the bytes into "y". Each byte is shifted left by multiplying it by 256 before adding the next
    * byte. */
   y  = y * 256;
   y += buf[6];
   y  = y * 256;
 ...
 */
static int bspatch(struct delta_api_t    *delta_apply,
                   int64_t                new_size,
                   uint8_t               *patch,
                   ...
                   decoder_state_t       *state)
{
   ...
   {
      /* Decompress the control data (ctrl[0] = diff, ctrl[1] = extra, ctrl[2] = offset in source firmware) */
      if(*bytes_written == 0)
      {
         ret = stream->decompress(stream,
                                  delta_apply,
                                  patch,
                                  CONTROL_DATA_SIZE * CONTROL_DATA_NB,
                                  stream->hsd,
                                  bytes_written,
                                  state);
         if(ret != 0)
      ...
      /* Sanity-check */
      if(ctrl[0] < 0 || ctrl[0] > INT_MAX || ctrl[1] < 0 || ctrl[1] > INT_MAX || newpos + ctrl[0] > new_size)
      {
      ...
      while(ctrl[0] > 0)
      {
         ...
         if(*bytes_written == 0)
         {
            ...
            ret = stream->decompress(stream, delta_apply, patch, decoded, stream->hsd, bytes_written, state);
            if(ret != 0)
            {
               LOG_ERR("Error decompressing patch for ctrl[0]");
            }
            /* If we decompressed less bytes than the maximum possible that means we decompressed all the data
             * for this ctrl[0] block */
         ...
         *bytes_written           -= bytes_to_read;
         ...
         if(ret != 0)
         {
            LOG_ERR("Can not read %d bytes in slot0 partition at offset : %lld, ret = %d\n",
                    bytes_to_read,
                    delta_apply->memory.offset.source,
                    ret);
            return ret;
         }
         ...
         ret      = delta_apply->seek(&delta_apply->memory, oldpos, delta_apply->memory.offset.patch, newpos);
         if(ret != 0)
         {
            LOG_ERR("Can not seek at offset for source : %d and for patch : %lld, ret = %d\n",
                    oldpos,
                    delta_apply->memory.offset.patch,
                    ret);
            return ret;
         }
      }
      ...
      while(ctrl[1] > 0)
      {
         ...
         *bytes_written -= bytes_to_read;
         ...
         ret             = delta_apply->write(&delta_apply->memory,
                                  new,
                                  bytes_to_read,
